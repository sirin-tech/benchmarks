defmodule Neuro.Nodes.Convolution do
  ...
  def __batch__(%{assigns: %{vars: vars}}) do
    #[{:run, {"infer", {vars.wx, vars.wy, vars.z}, {vars.oz, vars.oy, vars.ox}, [:shared]}}]
  ...
  defp inference_ptx() do
    """
    <%= defkernel ctx, "infer", shared: u64.ptr do %>
      .shared .<%= var(:f) %> result;
      .reg .<%= var(:f) %> %w, %i;
      .reg .u64 %x, %y, %z, %gx, %gy, %gz, %w_ptr, %i_ptr, %o_ptr, %b_ptr, %tmp;
      .reg .pred first_weight, p;
      <%= if training?() do %>
        .reg .u64 %states_ptr;
      <% end %>
      ...
      cvt.u64.u32   %y, %tid.y;
      cvt.u64.u32   %z, %tid.z;
      cvt.u64.u32   %gx, %ctaid.z;
      cvt.u64.u32   %gy, %ctaid.y;
      cvt.u64.u32   %gz, %ctaid.x;

      setp.eq.u64     first_weight, %x, 0;
      setp.eq.and.u64 first_weight, %y, 0, first_weight;
      setp.eq.and.u64 first_weight, %z, 0, first_weight;

      ld.param.u64    %i_ptr, [pins];
      ld.param.u64    %w_ptr, [shared];

      mad.lo.u64      %b_ptr, %gz, <%= var(:float_size) %>, %w_ptr;
      <%= if shared_offset(:biases) > 0 do %>
        add.u64       %b_ptr, %b_ptr, <%= shared_offset(:biases) %>;
      <% end %>

      mad.lo.u64    %tmp, %gy, <%= var(:ox) %>, %gx;
      mad.lo.u64    %tmp, %gz, <%= var(:ox) * var(:oy) %>, %tmp;
      <%= if training?() do %>
        mad.lo.u64  %states_ptr, %tmp, <%= var(:float_size) %>, %w_ptr;
        <%= if shared_offset(:states) > 0 do %>
          add.u64   %states_ptr, %states_ptr, <%= shared_offset(:states) %>;
        <% end %>
      <% end %>
      mad.lo.u64    %o_ptr, %tmp, <%= var(:float_size) %>, %i_ptr;
      <%= if pin_offset(:output) > 0 do %>
        add.u64     %o_ptr, %o_ptr, <%= pin_offset(:output) %>;
      <% end %>

      mad.lo.u64      %tmp, %y, <%= var(:wx) %>, %x;
      mad.lo.u64      %tmp, %gz, <%= var(:wx) * var(:wy) %>, %tmp;
      mad.lo.u64      %w_ptr, %tmp, <%= var(:float_size) %>, %w_ptr;
      ...
      <%= if var(:padding) do %>
        ...
        setp.lo.u64     p, %gx, <%= var(:px) %>;
        @!p bra         no_padding_1;
        add.u64         %tmp, %gx, %x;
        setp.lo.u64     p, %tmp, <%= var(:px) %>;
        @p bra          reduce;
      no_padding_1:
        setp.lo.u64     p, %gy, <%= var(:py) %>;
        @!p bra         no_padding_2;
        add.u64         %tmp, %gy, %y;
        setp.lo.u64     p, %tmp, <%= var(:py) %>;
        @p bra          reduce;
      no_padding_2:
        setp.hs.u64     p, %gx, <%= var(:ox) - var(:px) %>;
        @!p bra         no_padding_3;
        add.u64         %tmp, %gx, %x;
        setp.hi.u64     p, %tmp, <%= var(:ox) - var(:px) %>;
        @p bra          reduce;
      no_padding_3:
        setp.hs.u64     p, %gy, <%= var(:oy) - var(:py) %>;
        @!p bra         no_padding_4;
        add.u64         %tmp, %gy, %y;
        setp.hi.u64     p, %tmp, <%= var(:oy) - var(:py) %>;
        @p bra          reduce;
      no_padding_4:
      <% end %>

    load_input:
      // input = (ctaid.x * sx + tid.x + ctaid.y * sy * ix + tid.y * ix + tid.z * ix * iy / iz)
      mad.lo.u64    %tmp, %gx, <%= var(:sy) %>, %x;
      mad.lo.u64    %tmp, %gy, <%= var(:sy) * var(:x) %>, %tmp;
      mad.lo.u64    %tmp, %y, <%= var(:x) %>, %tmp;
      mad.lo.u64    %tmp, %z, <%= var(:x) * var(:y) %>, %tmp;
      <%= if var(:padding) do %>
      sub.u64       %tmp, %tmp, <%= var(:px) + var(:py) * var(:x) %>;
      <% end %>
      mad.lo.u64    %i_ptr, %tmp, <%= var(:float_size) %>, %i_ptr;
      <%= if pin_offset(:input) > 0 do %>
        add.u64     %i_ptr, %i_ptr, <%= pin_offset(:input) %>;
      <% end %>
      ld.global.<%= var(:f) %>  %i, [%i_ptr];
      <%= if var(:padding) and var(:pv) != 0.0 do %> reduce: <% end %>
      mul.rn.<%= var(:f) %>     %i, %i, %w;

      <%= if var(:padding) and var(:pv) == 0.0 do %> reduce: <% end %>
      @first_weight st.shared.<%= var(:f) %> [result], 0.0;
      bar.sync 1;
      red.shared.add.<%= var(:f) %> [result], %i;

      ld.shared.<%= var(:f) %>  %i, [result];
      ...
      <%= if training?() do %>
        st.global.<%= var(:f) %> [%states_ptr], %i;
      <% end %>

      <%= include ctx, var(ctx, :activation), in: "i", pred: "p", f: var(:f) %>

      st.global.<%= var(:f) %>  [%o_ptr], %i;
      ret;
    <% end %>

    <%= defkernel ctx, "inference", shared: u64.ptr do %>
      .reg .u64   %x, %y, %z, %pins, %i_ptr, %o_ptr, %w_ptr, %b_ptr,
                  %cx, %cy, %cz, %py, %px1, %px2, %sx, %sy;
      .reg .<%= var(:f) %> %acc, %i, %w;
      .reg .pred  p;
      <%= if training?() do %>
        .reg .u64 %states_ptr;
      ...
      // w.offset = tid.z * wx * wy * z * float_size
      mad.lo.u64    %w_ptr, %z, <%= var(:z) * var(:wx) * var(:wy) * var(:float_size) %>, %w_ptr;
      <%= if shared_offset(:weights) > 0 do %>
        add.u64     %w_ptr, %w_ptr, <%= shared_offset(:weights) %>;
      <% end %>
    ...
    loop_x:
      ...
      add.u64       %i_ptr, %i_ptr, <%= (var(:y) - var(:wy)) * var(:x) * var(:float_size) %>;
