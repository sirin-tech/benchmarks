defmodule Neuro.Layers.Convolution do
  ...
  @convolution Nodes.Convolution
  @pooling Nodes.Pooling
  ...
  def __graph__(%{assigns: %{back_propagation: true}} = graph) do
    ...
    {graph, next} = case vars.pooling do
      false -> {graph, :output}
      _     -> {graph |> chain(:pooling_node, @pooling), {:pooling_node, :input}}
    end
    graph = graph
            |> add(:conv_node, @convolution)
  ...
  def __graph__(graph) do
    vars = graph.assigns.vars
    graph = graph |> chain(:conv_node, @convolution)
    graph = case vars.pooling do
      false -> graph
      _     -> graph |> chain(:pooling_node, @pooling)
    end
    graph |> close()
  end
  ...
  defp process_conv(vars, env) do
    ...
    vars
    |> Map.put(:conv, c)
    |> Map.put(:conv_vars, @convolution.vars(c, env))
  end
  ...
  defp process_pooling(%{pooling: p, conv_vars: cv} = vars, env) do
    with p when is_list(p) <- pooling(p) do
      ...
      vars
      |> Map.put(:pooling, p)
      |> Map.put(:pooling_vars, @pooling.vars(p, env))
    else
